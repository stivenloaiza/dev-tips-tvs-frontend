...
const Tips = ({ level, technology }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const url = `http://localhost:3000/v1/api/tips?level=${level}&technology=${technology}`;
        const response = await axios.get(url);
        const data = response.data;
        setTipData(data);
      } catch (error) {
        console.error('Error fetching data:', error);
        // Muestra más detalles del error si es posible
  ...
  if (!tipData) {
    return <div>Loading...</div>; // Mostrar un indicador de carga mientras se carga la data
  } // Asegúrate de incluir level y technology como dependencias para que se actualice cuando cambien
